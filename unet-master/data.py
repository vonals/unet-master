...
def adjustData(img, mask, flag_multi_class, num_class):
    if(flag_multi_class):
        ...
        for i in range(num_class):
            # 对于每个像素, find the class in mask and convert it into one-hot vector
...
def trainGenerator(batch_size, train_path, image_folder, mask_folder, aug_dict, image_color_mode = "grayscale",
    ...
    '''
    image_datagen = ImageDataGenerator(**aug_dict)  # 创建ImageDataGenerator生成器
    ...
    for (img, mask) in train_generator:
        img, mask = adjustData(img, mask, flag_multi_class, num_class)  # 调整图片
        yield (img, mask)
